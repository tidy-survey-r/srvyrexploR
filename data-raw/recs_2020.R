...
recs <- recs_in %>%
  select(DOEID, REGIONC, DIVISION, STATE_FIPS, state_postal, state_name, UATYP10, TYPEHUQ, YEARMADERANGE, HEATHOME, HEATCNTL, TEMPHOME, TEMPGONE, TEMPNITE, AIRCOND, COOLCNTL, TEMPHOMEAC, TEMPGONEAC, TEMPNITEAC, TOTCSQFT, TOTHSQFT, TOTSQFT_EN, NWEIGHT, starts_with("NWEIGHT"),
    CDD30YR = CDD30YR_PUB, CDD65, BA_climate, IECC_climate_code, HDD30YR = HDD30YR_PUB, HDD65,
...
recs %>%
  count(state_postal, state_name, STATE_FIPS) %>%
  print(n = 51)

recs_out <- recs %>%
  select(
    DOEID, starts_with("NWEIGHT"),
...
cb_slim <- cb_ord %>%
  select(Variable = BookDerived, `Description and Labels`, Question, Section, Order) %>%
